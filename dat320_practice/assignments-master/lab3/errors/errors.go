...
func (m Errors) Error() string {
	...
	if len(errSlice) == 0 {
		return "(0 errors)"
	} else if len(errSlice) == 1 {
		return errSlice[0]
	} else if len(errSlice) == 2 {
		first := errSlice[0]
		return fmt.Sprintf("%v (and %v other error)", first, len(errSlice)-1)
	} else {
		first := errSlice[0]
		return fmt.Sprintf("%v (and %v other errors)", first, len(errSlice)-1)
	}
}
