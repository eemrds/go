...
func scramble(text string, seed int64) string {
	...
	var scrambledSentence []string
	for _, word := range tokenized {
		wordByte := []byte(word)
		if _, err := strconv.Atoi(word); err == nil {
			scrambledSentence = append(scrambledSentence, word)
			continue
		}
		if len(word) < 4 {
			scrambledSentence = append(scrambledSentence, word)
		...
		words := wordByte[1 : len(wordByte)-1]
		rand.Shuffle(len(words), func(i, j int) {
			words[i], words[j] = words[j], words[i]
		})

		scrambledWord := fmt.Sprintf("%v%v%v", word[:1], string(words), word[len(word)-1:])
		scrambledSentence = append(scrambledSentence, scrambledWord)
	}
	return strings.Join(scrambledSentence, "")
}
